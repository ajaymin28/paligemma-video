...
if __name__ == "__main__":
    ...
    RUN_NAME = f"gemma-3-4b-it-lora_r{config.LORA_RANK}_a{config.LORA_ALPHA}_lr{config.LEARNING_RATE}_justlora_imgmask"
    ...
    train_dataloader = DataLoader(train_dataset,
                                  collate_fn=lambda examples: collate_fn_video_gemma3(examples, 
                                                                                      processor, 
                                                                                      video_data_root, 
                                                                                      config.DEVICE, 
    ...
    for epoch in range(config.EPOCHS):
        for idx, batch in enumerate(train_dataloader):
            ...
            if (idx + 1) % config.GRADIENT_ACCU_STEPS == 0:
                optimizer.step()
                optimizer.zero_grad()
                accumulated_loss = 0
            ...
            if idx % 100 == 0:
                ...
                mem_usage = memory_stats(custom_label=f"After batch {idx}", get_dict=True)
                log_data = {
                    "train/loss": loss.item(), 
                    "train/epoch": epoch,
                }
                log_data.update(mem_usage)
            ...
            torch.cuda.empty_cache()
            # memory_stats(custom_label="After cuda empty cache")
