...
def collate_fn_video_test(batch_data,processor, device):
    ...
    for batch in batch_data:
        ...
        video, num_frames_to_sample = process_video_with_decord(video_path,frame_indices_custom=block_data["frame_idxes"])
...
def get_model_output(batch_data):
    ...
    inputs = collate_fn_video_test(batch_data, processor=processor,device=config.DEVICE)
    input_len = inputs["input_ids"].shape[-1] ## padded input length for the batch

    batch_outputs = []
    with torch.inference_mode():
        generation = model.generate(**inputs, max_new_tokens=2000, do_sample=False,  cache_implementation="dynamic")
...
def parse_args():
    ...
    parser.add_argument("--model-path", type=str, default="/home/ja882177/dso/gits/paligemma-video/checkpoints/paligemma2-lora_r256_a512_lr1e-5_with_mm_proj")
    ...
    parser.add_argument("--samples_to_process", type=int, default=1000,required=False)
    # parser.add_argument("--prev_eval_data", type=str, default="", required=False)
    return parser.parse_args()

def getAGAnnotationsBatch(AG_Annotations):    
    all_video_samples = []
...
if __name__=="__main__":
    ...
    ALL_VIDEO_SAMPLES = getAGAnnotationsBatch(AG_Annotations=AG_Annotations)
